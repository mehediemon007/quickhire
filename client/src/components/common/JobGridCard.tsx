...
function JobGridCard({ job }: { job: Job }) {
    return (
        <div className='relative h-full border border-neutral-200 p-6'>
            <Link href={`/jobs/${job.id}`} className="absolute inset-0 z-0" aria-label={job.title} />
            <div className="col-span-1 relative z-10 pointer-events-none">
                <div className='relative w-12 h-12 aspect-square mb-4'>
                    <Image src={job.logo} alt={job.companyName} fill sizes='100vw' className='transition-colors duration-200 ease-in group-hover:brightness-0 group-hover:invert'/>
                </div>
                <div>
                    <h5><Link href={'#'}>{job.title}</Link></h5>
                ...
                <span className='absolute top-0 right-0 border border-primary px-3 py-1'>
                    {job.jobType}
                </span>
            </div>
        </div>
